...
The world uses an original 2.5D visual language: hand-painted terrain textures, cobbled town surfaces, taller textured buildings, thin connected wall segments, crenellated castle walls, raised rooflines, ambient cast shadows, and layered character silhouettes. Fortifications use heavy ashlar masonry, raised caps, alternating merlons, and narrow arrow slits. A synchronized three-minute atmosphere cycle moves through daylight, night, drifting fog, and rain; authored torches flicker and add local warm light, while underground floors remain dark. Every complete building is derived from one canonical tile-space footprint: full floor coverage, four wall edges, openings, glazed windows, wall tops, symmetric roof eaves, and a centred gable ridge are projected through the same camera function. Canonical walls are rendered as one-tile modules with fixed-scale local materials, repeated timber framing, and exactly one visual owner. House walls are never sent through the legacy renderer; legacy walls are limited to freestanding fortifications without a canonical building footprint. House collision uses the same thin footprint edges: tiles on both sides remain walkable, crossing an actual wall edge is blocked, and an open door replaces exactly one edge section. Server pathfinding and client prediction share this rule. Greyhaven's plaster-and-oak houses, clay roof shingles, roads, fortified stone, and full-sized oak trees use dedicated original materials that remain readable at the close fixed camera scale. Trees can be painted in the editor and are server-authoritative obstacles for players and creature pathfinding. Closed rectangular outlines made with the individual House wall brush are promoted to canonical buildings when loaded and automatically receive aligned floors, collision, walls, and roofs; synchronized doors count as valid perimeter openings. Doors have a visible frame and upright wooden leaf that changes orientation between open and closed states. Press F3 for geometry diagnostics, F4 for legacy fortifications, F5 for canonical walls, F6 for roofs, and F7 for canonical floors. These toggles allow every representation to be inspected in isolation; normal rendering never draws both wall systems for one building. Walls, trees, and roofs smoothly become translucent when they pass in front of the local player. Greyhaven is a server-authored medieval settlement with a fortified outer wall, two usable gates, a stone keep, timber houses with walkable interiors, a market square, and roads leading into the hunting lands. Buildings have roofs above world entities that fade while the local character is inside. House and keep doors are clickable, synchronized to every player, block players and creature pathfinding while closed, and cannot close on an occupied doorway. Greyhaven Keep has a persistent upper floor at z=6 and a dangerous cellar dungeon at z=8. Separate stairs connect both levels to the ground floor; stepping onto one changes the authoritative player floor, filters all world rendering and interaction to that floor, and survives reconnects through position persistence. The cellar has dark masonry, internal rooms and chokepoints, torchlight, and four original tiered enemies: Castle Rat, Crypt Guard, Bone Acolyte, and Cellar Warden. Each has distinct health, speed, damage, experience, corpse capacity, weighted physical loot, and four-direction artwork in a dedicated transparent sprite atlas. Walls, windows, torches, terrain materials, doors, floors, water, bridges, trees, roads, buildings, and stairs are shared through protocol 17 so rendering and authored content agree. It aims for the spatial readability and physical-world feeling of classic top-down MMORPGs while retaining the modern interface and original Aldoria art direction.

Players, creatures, and items use original hand-painted sprite atlases. The adventurer has idle and alternating walk poses in four directions, selected from smoothed movement while the feet remain grounded. Item artwork is shared by the world, inventory, shops, depot, loot, and direct trade UI. Mire creatures have north, east, south, and west frames; the client selects direction from confirmed movement and adds a lightweight procedural walk motion without changing server-authoritative positions or hitboxes. Code-drawn shapes remain as a fallback if an atlas cannot load.
