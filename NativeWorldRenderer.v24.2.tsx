...
// Smooth movement plus shortest-arc facing interpolation. Never interpolate
// yaw as an ordinary scalar across the -PI/+PI wrap boundary.
...
export const NativeWorldRenderer = memo(function NativeWorldRenderer({
  ...
  useEffect(() => {
    ...
    console.info(
      "NATIVE WORLD V24.2 active · smooth movement · shortest-arc facing",
    );
        ...
        if (map && local) {
          ...
          if (positionRef.current) {
            positionRef.current.textContent =
              `NATIVE V24.2 · x ${local.position.x} · y ${local.position.y} · z ${floor}`;
          }
        }
    ...
    void bootstrap().catch((error) => {
      console.error("Native V24.2 renderer bootstrap failed", error);
    });
