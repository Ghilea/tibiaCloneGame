...
use crate::{MovementState, creature_sprites, native_ui_theme as theme, streaming};
...
impl NativeLoadingState {
    ...
    pub(crate) fn update_render_pipeline_status(&mut self, total: usize, waiting: usize) {
        let was_ready = self.render_pipelines_ready();

        let counts_changed =
            total != self.render_pipeline_total || waiting != self.render_pipeline_waiting;

        self.render_pipeline_probe_seen = true;
        self.render_pipeline_total = total;
        self.render_pipeline_waiting = waiting;

        if total > 0 && waiting == 0 && total == self.render_pipeline_last_total {
            self.render_pipeline_stable_frames =
                self.render_pipeline_stable_frames.saturating_add(1);
        } else {
        ...
        if counts_changed {
            info!(
                ...
                total, waiting, self.render_pipeline_stable_frames,
            );
        }

        if !was_ready && self.render_pipelines_ready() {
            info!(
                "ALDORIA GPU PIPELINES READY · total={} · waiting=0 · stable={}",
                total, self.render_pipeline_stable_frames,
            );
        }
    }

    pub(crate) fn render_pipelines_ready(&self) -> bool {
        ...
        !self.render_pipeline_probe_seen
            || (self.render_pipeline_total > 0
                ...
                && self.render_pipeline_stable_frames >= 12)
    }

    pub(crate) fn render_pipeline_waiting(&self) -> usize {
        self.render_pipeline_waiting
    }

    pub(crate) fn render_pipeline_total(&self) -> usize {
...
pub(crate) fn gameplay_ready(state: Option<Res<NativeLoadingState>>) -> bool {
...
pub(crate) fn setup(
    ...
    launcher: Option<Res<crate::native_launcher::NativeLauncherState>>,
) {
    if launcher.is_some() {
    ...
    commands
        .spawn((
            ...
            BackgroundColor(Color::srgba(0.02, 0.03, 0.04, 0.18)),
        ))
        .with_children(|root| {
            root.spawn((
                Node {
            ...
            .with_children(|card| {
                ...
                card.spawn((
                    Text::new("World cache | creature assets | interface"),
                    TextFont {
...
pub(crate) fn update_render_pipeline_readiness(
    ...
    let total = pipelines.pipelines().count();

    let waiting = pipelines.waiting_pipelines().count();

    if let Some(mut loading) = main_world.get_resource_mut::<NativeLoadingState>() {
        loading.update_render_pipeline_status(total, waiting);
    }
}
...
pub(crate) fn update(
    ...
    streamed_entities: Query<&streaming::StreamedRegionEntity>,
    streamed_renderables: Query<(&streaming::StreamedRegionEntity, &ViewVisibility), With<Mesh3d>>,
    ...
    mut overlay: Query<&mut Visibility, With<NativeLoadingOverlay>>,
    mut status: Query<&mut Text, With<NativeLoadingStatus>>,
    mut progress: Query<&mut Node, With<NativeLoadingProgress>>,
) {
    if !state.active {
        return;
    }

    state.visible_seconds += time.delta().as_secs_f32();

    let floor = movement.logical.z;
    let world_ready = stream.floor_ready(floor);
    let active_generation = stream.active_generation();

    let streamed_entity_count = streamed_entities
        .iter()
        .filter(|entity| entity.belongs_to_active_floor(active_generation, floor))
        .count();

    if streamed_entity_count == state.last_streamed_entity_count && streamed_entity_count > 0 {
        state.stable_stream_frames = state.stable_stream_frames.saturating_add(1);
    } else {
        state.stable_stream_frames = 0;
        state.last_streamed_entity_count = streamed_entity_count;
    }

    let streamed_entities_ready =
        world_ready && streamed_entity_count > 0 && state.stable_stream_frames >= 4;

    let visible_world_meshes = streamed_renderables
        .iter()
        .filter(|(entity, visibility)| {
            entity.belongs_to_active_floor(active_generation, floor) && visibility.get()
        })
        .count();
    ...
    for root in &actor_roots {
        let meshes = count_descendant_meshes(root, &children, &mesh_visibility);
    ...
    let presentation_unchanged = visible_world_meshes == state.last_visible_world_meshes
    ...
    if presentation_unchanged {
        state.stable_presentation_frames = state.stable_presentation_frames.saturating_add(1);
    } else {
    ...
    let actors_instantiated = actor_roots_with_mesh == actor_root_count;
    let presentation_stable = actors_instantiated && state.stable_presentation_frames >= 120;

    let creature_assets_ready =
        creature_sprites::floor_transition_creature_assets_ready(&asset_server, &creature_catalog);
    ...
    for (entity, root, instance, animation_template) in &world_asset_roots {
        ...
        world_asset_dependencies_ready &= dependencies_ready;

        if dependencies_ready && instance.is_some() {
    ...
    let render_visible = visible_world_meshes > 0 && visible_player_meshes > 0;

    if render_visible {
        state.stable_render_frames = state.stable_render_frames.saturating_add(1);
    } else {
        state.stable_render_frames = 0;
    }

    let render_visibility_ready =
        state.stable_render_frames >= 12 && actors_instantiated && presentation_stable;
    ...
    let (stage, label, width) = if !world_ready {
        ...
        (
            "world",
            format!("Loading world cache for floor {floor}..."),
            30.0,
        )
        ...
        state.ready_seconds += time.delta().as_secs_f32();

        ("render", "Finalizing rendered world...".to_owned(), 97.0)
    };
