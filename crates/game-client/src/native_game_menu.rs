...
use crate::{
    native_map_ui::NativeMapUiState, native_modal, native_settings::NativeSettingsState,
    native_ui::NativePanelState, native_ui_theme as theme, state::NativeGameState,
};
...
pub(crate) fn setup(mut commands: Commands) {
    commands.insert_resource(NativeGameMenuState::default());
    ...
    commands
        ...
        .with_children(|root| {
            root.spawn((
                Name::new("Native modal surface · game menu"),
                native_modal::NativeModalSurface,
                native_modal::panel_node(620.0, 430.0),
...
pub(crate) fn handle_input(
    ...
    let another_modal_open = settings.options_open
...
pub(crate) fn update_ui(
    ...
    game_state: Res<NativeGameState>,
    mut roots: Query<&mut Visibility, With<NativeGameMenuRoot>>,
    mut context: Query<&mut Text, With<NativeGameMenuContext>>,
) {
    ...
    let value = if let Some(player) = game_state.local_player() {
        format!(
            "{}  |  Level {}  |  {}:{}:{}",
            player.name, player.level, player.position.x, player.position.y, player.position.z,
        )
    } else {
        "Greyhaven".to_owned()
    };
...
pub(crate) fn menu_closed(menu: Option<Res<NativeGameMenuState>>) -> bool {
    match menu {
        Some(menu) => !menu.open,
        None => true,
    }
