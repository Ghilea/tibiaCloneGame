...
pub(crate) struct NativeDraggableSurface(
    pub(crate) NativeModalWindow,
);

#[derive(Component, Clone, Copy)]
pub(crate) struct NativeDragHandle(
    pub(crate) NativeModalWindow,
);
...
pub(crate) fn handle_window_drag(
    ...
    *offset += delta;
    ...
    let max_x =
        (window.width() * 0.72).max(120.0);

    let max_y =
        (window.height() * 0.62).max(90.0);

    offset.x =
        offset.x.clamp(-max_x, max_x);

    offset.y =
        offset.y.clamp(-max_y, max_y);
