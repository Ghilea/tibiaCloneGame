...
export const NativeWorldRenderer = memo(function NativeWorldRenderer({
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      ...
      console.info(
        `NATIVE V29 dynamic lights: ${dynamicLightsEnabled ? "enabled" : "disabled"}`,
      );
        ...
        if (map && local) {
          ...
          if (positionRef.current) {
            positionRef.current.textContent =
              `NATIVE V30.2 · x ${local.position.x} · y ${local.position.y} · z ${floor}`;
          }
        }
    ...
    void bootstrap().catch((error) => {
      console.error("Native V30.2 renderer bootstrap failed", error);
    });
