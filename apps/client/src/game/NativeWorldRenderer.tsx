...
type NativeAuthoredWorldPropKind =
  (typeof NATIVE_AUTHORED_WORLD_PROP_KINDS)[number];

const NATIVE_AUTHORED_WORLD_PROP_KIND_SET =
  new Set<WorldObjectKind>(NATIVE_AUTHORED_WORLD_PROP_KINDS);
...
type StaticSnapshot = {
  ...
  authoredProps: Record<NativeAuthoredWorldPropKind, Transform[]>;
  torches: Transform[];
...
type NativeAuthoredModelAsset = {
  scenes: readonly THREE.Group[];
  parts: readonly NativeGltfPart[];
};

type NativeWorldPropAssets = {
  props: Record<NativeAuthoredWorldPropKind, NativeAuthoredModelAsset>;
  copperVein: NativeAuthoredModelAsset;
  copperVeinDepleted: NativeAuthoredModelAsset;
};

const NATIVE_WORLD_PROP_MODEL_PATHS:
  Record<NativeAuthoredWorldPropKind, string> = {
    chair: "/assets/models/world-props/chair.glb",
    table: "/assets/models/world-props/table.glb",
    bench: "/assets/models/world-props/bench.glb",
    well: "/assets/models/world-props/well.glb",
    barrel: "/assets/models/world-props/barrel.glb",
    notice_post: "/assets/models/world-props/notice_post.glb",
    wrecked_planks: "/assets/models/world-props/wrecked_planks.glb",
    wooden_crate: "/assets/models/world-props/wooden_crate.glb",
    grain_sack: "/assets/models/world-props/grain_sack.glb",
    bone_pile: "/assets/models/world-props/bone_pile.glb",
    rock_pile: "/assets/models/world-props/rock_pile.glb",
    mushroom_patch: "/assets/models/world-props/mushroom_patch.glb",
    campfire: "/assets/models/world-props/campfire.glb",
    hay_bundle: "/assets/models/world-props/hay_bundle.glb",
    fence_post: "/assets/models/world-props/fence_post.glb",
  };
...
class NativeAuthoredInstancedLayer {
  readonly set: NativeGltfInstancedSet;

  constructor(
    readonly asset: NativeAuthoredModelAsset,
    capacity: number,
  ) {
    this.set = new NativeGltfInstancedSet(asset.parts, capacity);
  }

  get meshes() {
    return this.set.meshes;
  }

  setTransforms(transforms: readonly Transform[]) {
    this.set.setTransforms(this.asset.parts, transforms);
  }
}
...
function extractNativeGltfPartsFromRoot(
  root: THREE.Object3D,
): readonly NativeGltfPart[] {
  ...
  if (!parts.length) {
    throw new Error("Native V31B: authored GLB contains no mesh parts");
  }
  return parts;
}

function normalizeNativeAuthoredParts(
  parts: readonly NativeGltfPart[],
  ...
  for (const part of parts) {
    ...
    if (!box) continue;
    transformed.copy(box).applyMatrix4(part.localMatrix);
  ...
  return parts.map((part) => ({
    ...part,
    localMatrix: normalize.clone().multiply(part.localMatrix),
  }));
}

async function loadNativeAuthoredModel(
  path: string,
  targetFootprint: number,
  targetHeight: number,
): Promise<NativeAuthoredModelAsset> {
  const gltf = await new GLTFLoader().loadAsync(path);
  const scenes = gltf.scenes.length > 0
    ? gltf.scenes
    : [gltf.scene];
  const root = gltf.scene;
  const parts = normalizeNativeAuthoredParts(
    extractNativeGltfPartsFromRoot(root),
    targetFootprint,
    targetHeight,
  );
  return { scenes, parts };
...
const NATIVE_WORLD_PROP_FIT:
  Record<NativeAuthoredWorldPropKind, readonly [number, number]> = {
    chair: [0.72, 1.15],
    table: [0.92, 0.84],
    bench: [0.96, 1.00],
    well: [0.96, 1.75],
    barrel: [0.64, 0.74],
    notice_post: [0.78, 1.55],
    wrecked_planks: [0.92, 0.28],
    wooden_crate: [0.68, 0.62],
    grain_sack: [0.58, 0.72],
    bone_pile: [0.78, 0.34],
    rock_pile: [0.82, 0.58],
    mushroom_patch: [0.76, 0.62],
    campfire: [0.76, 0.48],
    hay_bundle: [0.82, 0.62],
    fence_post: [0.92, 1.12],
  };

async function loadNativeWorldPropAssets(): Promise<NativeWorldPropAssets> {
  const entries = await Promise.all(
    NATIVE_AUTHORED_WORLD_PROP_KINDS.map(async (kind) => {
      const [footprint, height] = NATIVE_WORLD_PROP_FIT[kind];
      const asset = await loadNativeAuthoredModel(
        NATIVE_WORLD_PROP_MODEL_PATHS[kind],
        footprint,
        height,
      );
  ...
  const [
    ...
    loadNativeAuthoredModel(
      "/assets/models/world-props/copper_vein.glb",
      0.84,
      0.86,
    ),
    loadNativeAuthoredModel(
      "/assets/models/world-props/copper_vein_depleted.glb",
      0.84,
      0.58,
    ),
  ]);
  ...
  return {
    props: Object.fromEntries(entries) as Record<
      NativeAuthoredWorldPropKind,
      NativeAuthoredModelAsset
    >,
    copperVein,
    copperVeinDepleted,
  };
...
function disposeNativeAuthoredModelAsset(asset: NativeAuthoredModelAsset) {
  ...
  for (const scene of asset.scenes) {
    scene.traverse((object) => {
      ...
      const materialEntries = Array.isArray(object.material)
        ? object.material
        : [object.material];
...
function disposeNativeWorldPropAssets(assets: NativeWorldPropAssets) {
  for (const asset of Object.values(assets.props)) {
    disposeNativeAuthoredModelAsset(asset);
  }
  disposeNativeAuthoredModelAsset(assets.copperVein);
  disposeNativeAuthoredModelAsset(assets.copperVeinDepleted);
}
...
function prepareNativeSnapshot(
  ...
  appendNativeConnectedWalls(
    ...
    HOUSE_WALL_HEIGHT,
    0.24,
    snapshot.looseHouseWalls,
    (position) => buildingWallOwnsPosition(position, map.buildings),
  );
  ...
  for (const object of map.objects ?? []) {
    ...
    if (NATIVE_AUTHORED_WORLD_PROP_KIND_SET.has(object.kind)) {
      const kind = object.kind as NativeAuthoredWorldPropKind;
      const naturalRotation = [
        ...
        ? ((object.position.x * 13 + object.position.y * 7) % 4)
          * Math.PI / 2
        : 0;

      snapshot.authoredProps[kind].push([
        object.position.x + 0.5,
        0,
        object.position.y + 0.5,
        1,
...
function initializeNativeLoadedAssetTextures(
  ...
  collectRoots(Object.values(characters.templates));
  collectRoots(medieval.scenes);
  for (const asset of Object.values(worldProps.props)) {
    collectRoots(asset.scenes);
  }
  collectRoots(worldProps.copperVein.scenes);
  collectRoots(worldProps.copperVeinDepleted.scenes);

  for (const atlas of creatures.atlases.values()) {
...
type NativeDynamicLayers = {
  ...
  groundWeapons: NativeInstancedLayer;
  groundArmor: NativeInstancedLayer;
  groundContainers: NativeInstancedLayer;
  ...
  resourceCopperAuthored: NativeAuthoredInstancedLayer;
  resourceCopperDepletedAuthored: NativeAuthoredInstancedLayer;
  ...
  projectilePhysical: NativeInstancedLayer;
  projectileMagic: NativeInstancedLayer;
  npcShopMarkers: NativeInstancedLayer;
  npcDepotMarkers: NativeInstancedLayer;
  npcSpellMarkers: NativeInstancedLayer;
  npcCraftMarkers: NativeInstancedLayer;
...
function nativeGroundItemFamily(
  ...
  if (
    ...
    || identity.includes("bow")
    || identity.includes("crossbow")
    || identity.includes("wand")
  ...
  if (
    ...
    || identity.includes("bag")
    || identity.includes("backpack")
    || identity.includes("chest")
    || identity.includes("box")
  ) {
    return "container";
  }
  ...
  if (
    ...
    || identity.includes("shield")
    || identity.includes("armor")
    || identity.includes("helmet")
    || identity.includes("legs")
    || identity.includes("boots")
  ) {
    return "armor";
  }
...
type NativeProjectileFamily = "physical" | "magic" | null;

function nativeProjectileFamily(
  effectId: string,
  spellKnown: boolean,
  ...
  if (spellKnown || nativeSpellEffectFamily(effectId)) {
    return "magic";
  }
...
class NativeDynamicSceneManager {
  ...
  setHover(
    ...
    this.hoverMaterial.color.set(hoverColor);
  }
  ...
  private updateGroundAndResources(
    ...
    const weapons: Transform[] = [];
    const armor: Transform[] = [];
    const containers: Transform[] = [];
    ...
    const copperAuthored: Transform[] = [];
    const copperDepletedAuthored: Transform[] = [];
    ...
    this.layers.groundWeapons.setTransforms(weapons);
    this.layers.groundArmor.setTransforms(armor);
    this.layers.groundContainers.setTransforms(containers);
    ...
    this.layers.resourceCopperAuthored.setTransforms(copperAuthored);
    this.layers.resourceCopperDepletedAuthored.setTransforms(
      copperDepletedAuthored,
    );
  ...
  private updateEffects(
    ...
    const physicalProjectiles: Transform[] = [];
    const magicProjectiles: Transform[] = [];
    ...
    for (const effect of world.combatEffects) {
      ...
      const projectileFamily = nativeProjectileFamily(
        effect.effectId,
        world.spells.has(effect.effectId),
      );
      const source = this.logicalEntityPosition(world, effect.sourceId);
      if (
        projectileFamily
        && source
        && source.z === floor
        ...
        const travel = THREE.MathUtils.smoothstep(age / 260, 0, 1);
        const startX = source.x + 0.5;
        const startZ = source.y + 0.5;
        const endX = effect.position.x + 0.5;
        const endZ = effect.position.y + 0.5;
        const dx = endX - startX;
        const dz = endZ - startZ;
        const px = THREE.MathUtils.lerp(startX, endX, travel);
        const pz = THREE.MathUtils.lerp(startZ, endZ, travel);
        const py = 0.78 + Math.sin(travel * Math.PI) * 0.48;

        if (projectileFamily === "physical") {
          physicalProjectiles.push([
            px,
            py,
            pz,
            1,
          ...
          const projectileScale = 0.72 + Math.sin(travel * Math.PI) * 0.24;
    ...
    this.layers.projectilePhysical.setTransforms(physicalProjectiles);
    this.layers.projectileMagic.setTransforms(magicProjectiles);
  ...
  private updateNpcMarkers(
    ...
    const shop: Transform[] = [];
    const depot: Transform[] = [];
    const spell: Transform[] = [];
    const craft: Transform[] = [];

    for (const npc of world.npcs.values()) {
      if (npc.position.z !== floor) continue;
      const visual = actorManager.entityVisualPosition(npc.id);
      const x = visual?.x ?? npc.position.x + 0.5;
      const z = visual?.z ?? npc.position.y + 0.5;
      const bob = 2.48 + Math.sin(now * 0.0035 + npc.position.x) * 0.07;
      const pulse = 0.92 + Math.sin(now * 0.004 + npc.position.y) * 0.08;
      const transform: Transform = [
        x,
        bob,
        z,
      ...
      switch (npc.service) {
        case "shop":
          ...
          break;
        case "depot":
          depot.push(transform);
          break;
        case "spell_trainer":
          spell.push(transform);
          break;
        case "craft_trainer":
          craft.push(transform);
          break;
      }
    }

    this.layers.npcShopMarkers.setTransforms(shop);
    this.layers.npcDepotMarkers.setTransforms(depot);
    this.layers.npcSpellMarkers.setTransforms(spell);
    this.layers.npcCraftMarkers.setTransforms(craft);
  }
...
export const NativeWorldRenderer = memo(function NativeWorldRenderer({
  ...
  useEffect(() => {
    ...
    let loadedWorldPropAssets: NativeWorldPropAssets | null = null;
...
// TIBIAGAME_V35_5_NETWORK_FRAME_BUDGET
    ...
    const bootstrap = async () => {
      ...
      const groundWeaponGeometry = new THREE.BoxGeometry(0.92, 0.08, 0.12);
      const groundArmorGeometry = new THREE.CylinderGeometry(
        0.28,
        0.34,
        0.08,
        10,
      );
      const groundContainerGeometry = new THREE.BoxGeometry(0.54, 0.34, 0.46);

      const projectilePhysicalGeometry = new THREE.CylinderGeometry(
        0.028,
        0.028,
        0.62,
        7,
      );
      projectilePhysicalGeometry.rotateZ(Math.PI / 2);
      const projectileMagicGeometry = new THREE.IcosahedronGeometry(0.16, 1);

      const npcMarkerGeometry = new THREE.OctahedronGeometry(0.14, 0);
      ...
      disposables.push(
        ...
        spellIceGeometry,
        spellArcaneGeometry,
        groundWeaponGeometry,
        groundArmorGeometry,
        groundContainerGeometry,
        projectilePhysicalGeometry,
        projectileMagicGeometry,
        npcMarkerGeometry,
      );
      ...
      const materials = {
        ...
        groundWeapon: new THREE.MeshStandardMaterial({
          color: "#b7bcc0",
          metalness: 0.68,
          roughness: 0.34,
        }),
        groundArmor: new THREE.MeshStandardMaterial({
          color: "#7f8a8d",
          metalness: 0.5,
          roughness: 0.46,
        }),
        groundContainer: new THREE.MeshStandardMaterial({
          color: "#765033",
          roughness: 0.88,
        }),
        ...
        projectilePhysical: new THREE.MeshStandardMaterial({
          color: "#d6c6a5",
          metalness: 0.22,
          roughness: 0.5,
        }),
        projectileMagic: new THREE.MeshBasicMaterial({
          color: "#c49aff",
          transparent: true,
          opacity: 0.92,
          depthWrite: false,
        ...
        npcShopMarker: new THREE.MeshBasicMaterial({
          color: "#f0c253",
          toneMapped: false,
        }),
        npcDepotMarker: new THREE.MeshBasicMaterial({
          color: "#72b8e8",
          toneMapped: false,
        }),
        npcSpellMarker: new THREE.MeshBasicMaterial({
          color: "#bc87f2",
          toneMapped: false,
        }),
        npcCraftMarker: new THREE.MeshBasicMaterial({
          color: "#e28a55",
          toneMapped: false,
        }),
      ...
      const layers = {
        ...
        groundWeapons: new NativeInstancedLayer(
          groundWeaponGeometry,
          materials.groundWeapon,
          512,
          false,
          true,
        ),
        groundArmor: new NativeInstancedLayer(
          groundArmorGeometry,
          materials.groundArmor,
          512,
          false,
          true,
        ),
        groundContainers: new NativeInstancedLayer(
          groundContainerGeometry,
          materials.groundContainer,
          512,
          false,
          true,
        ),
        ...
        projectilePhysical: new NativeInstancedLayer(
          projectilePhysicalGeometry,
          materials.projectilePhysical,
          32,
          false,
          false,
        ),
        projectileMagic: new NativeInstancedLayer(
          projectileMagicGeometry,
          materials.projectileMagic,
          32,
          false,
          false,
        ),
        npcShopMarkers: new NativeInstancedLayer(
          npcMarkerGeometry,
          materials.npcShopMarker,
          128,
          false,
          false,
        ),
        npcDepotMarkers: new NativeInstancedLayer(
          npcMarkerGeometry,
          materials.npcDepotMarker,
          128,
          false,
          false,
        ),
        npcSpellMarkers: new NativeInstancedLayer(
          npcMarkerGeometry,
          materials.npcSpellMarker,
          128,
          false,
          false,
        ),
        npcCraftMarkers: new NativeInstancedLayer(
          npcMarkerGeometry,
          materials.npcCraftMarker,
          128,
          false,
          false,
        ),
      ...
      layers.projectilePhysical.mesh.renderOrder = 21;
      layers.projectileMagic.mesh.renderOrder = 21;
      layers.npcShopMarkers.mesh.renderOrder = 22;
      layers.npcDepotMarkers.mesh.renderOrder = 22;
      layers.npcSpellMarkers.mesh.renderOrder = 22;
      layers.npcCraftMarkers.mesh.renderOrder = 22;
      ...
      const worldPropLayers = Object.fromEntries(
        NATIVE_AUTHORED_WORLD_PROP_KINDS.map((kind) => [
          kind,
          new NativeAuthoredInstancedLayer(
            worldPropAssets.props[kind],
            1024,
          ),
        ]),
      ) as Record<
        NativeAuthoredWorldPropKind,
        NativeAuthoredInstancedLayer
      >;
      for (const layer of Object.values(worldPropLayers)) {
        for (const mesh of layer.meshes) scene.add(mesh);
      }

      const copperVeinLayer = new NativeAuthoredInstancedLayer(
        worldPropAssets.copperVein,
        512,
      );
      const copperVeinDepletedLayer = new NativeAuthoredInstancedLayer(
        worldPropAssets.copperVeinDepleted,
        512,
      );
      for (const mesh of [
        ...copperVeinLayer.meshes,
        ...copperVeinDepletedLayer.meshes,
      ]) {
        scene.add(mesh);
      }
      ...
      dynamicSceneManager = new NativeDynamicSceneManager(
        scene,
        {
          ...
          groundWeapons: layers.groundWeapons,
          groundArmor: layers.groundArmor,
          groundContainers: layers.groundContainers,
          ...
          resourceCopperAuthored: copperVeinLayer,
          resourceCopperDepletedAuthored: copperVeinDepletedLayer,
          ...
          projectilePhysical: layers.projectilePhysical,
          projectileMagic: layers.projectileMagic,
          npcShopMarkers: layers.npcShopMarkers,
          npcDepotMarkers: layers.npcDepotMarkers,
          npcSpellMarkers: layers.npcSpellMarkers,
          npcCraftMarkers: layers.npcCraftMarkers,
      ...
      console.info(
        `NATIVE V31B dynamic lights: ${dynamicLightsEnabled ? "enabled" : "disabled"}`,
      );
      console.info(
        `NATIVE V31B authored world models: ${
          NATIVE_AUTHORED_WORLD_PROP_KINDS.length
        } props + copper vein states`,
      );
      ...
      const queueStaticSnapshot = (
        ...
        for (const kind of NATIVE_AUTHORED_WORLD_PROP_KINDS) {
          stage(`authored prop ${kind}`, () => {
            worldPropLayers[kind].setTransforms(
              snapshot.authoredProps[kind],
            );
          });
        }
      ...
      const targetAt = (position: Position): PointerTarget => {
        ...
        const door = world.map?.doors.find((entry) =>
        ...
        const window = world.map?.windows.find((entry) =>
          sameTile(entry.position)
        );
        if (window) {
          return {
            ...
            id: window.id,
            label: window.open ? "Close shutters" : "Open shutters",
          };
        }

        const object = world.map?.objects?.find((entry) =>
          sameTile(entry.position)
        );
        if (object) {
          return {
            ...
            id: object.id,
            label: `Inspect ${humanizeId(object.kind)}`,
          };
        }
        ...
        if (groundItem) {
          ...
          return {
            ...
            label: groundItem.contents.length > 0
      ...
      const activatePointerTarget = (
        ...
        switch (target.kind) {
          case "npc":
            ...
            return;
          case "creature":
            input.targetCreature(target.id);
            return;
          case "player":
          ...
          case "resource":
            ...
            return;
          case "door":
            input.toggleDoor(target.id, target.position);
            return;
          case "window":
            input.toggleWindow(target.id, target.position);
            return;
          case "object":
            input.interactWorldObject(target.id, target.position);
            return;
          case "loot":
            input.lootAt(target.position);
            return;
          case "ground":
            input.interactAt(target.position);
            return;
      ...
      const onPointerMove = (event: PointerEvent) => {
        ...
        const hoverKind = target.kind;
        activeDynamicSceneManager.setHover(target.position, hoverKind);
        canvas.style.cursor =
          target.kind === "resource" && !target.available ? "" : "pointer";

        if (hoverRef.current) {
          hoverRef.current.textContent = target.label;
      ...
      // V31B.2: force persistent geometry/materials and already-decoded actor
      // atlas textures onto the GPU while the loading overlay is still visible.
      const nativeGpuWarmupStartedAt = performance.now();
      const nativeWarmupTransform: Transform = [
        -10_000,
        -10_000,
        -10_000,
        1,
      ...
      for (const layer of Object.values(layers)) {
        layer.setTransforms([nativeWarmupTransform]);
      }

      medievalLayers.houseWalls.setTransforms(
        medievalAssets.wallParts,
        [nativeWarmupTransform],
      );
      medievalLayers.shuttersOpen.setTransforms(
        medievalAssets.shutterOpenParts,
        [nativeWarmupTransform],
      );
      medievalLayers.shuttersClosed.setTransforms(
        medievalAssets.shutterClosedParts,
        [nativeWarmupTransform],
      );

      for (const kind of NATIVE_AUTHORED_WORLD_PROP_KINDS) {
        worldPropLayers[kind].setTransforms([nativeWarmupTransform]);
      }
      copperVeinLayer.setTransforms([nativeWarmupTransform]);
    ...
    void bootstrap().catch((error) => {
      console.error("Native V31B renderer bootstrap failed", error);
    });
    ...
    return () => {
      ...
      if (loadedWorldPropAssets) {
        disposeNativeWorldPropAssets(loadedWorldPropAssets);
        loadedWorldPropAssets = null;
      }
    };
  }, [input, world]);
