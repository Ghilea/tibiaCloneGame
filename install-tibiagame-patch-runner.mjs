...
function findRepoRoot(start = process.cwd()) {
  ...
  while (true) {
    if (ROOT_MARKERS.every((entry) => existsSync(join(current, entry)))) return current;
    const parent = resolve(current, "..");
    if (parent === current) break;
    current = parent;
  }
  fail("Could not find the TibiaGame repository root.");
}

const root = findRepoRoot();
const packagePath = join(root, "package.json");
const scriptsDir = join(root, "scripts");
const runnerPath = join(scriptsDir, "apply-latest-patch.mjs");
...
try {
  ...
  fail("Could not parse root package.json.");
}
pkg.scripts ??= {};
pkg.scripts.patch = "node scripts/apply-latest-patch.mjs";
pkg.scripts["patch:check"] = "node scripts/apply-latest-patch.mjs --check";
writeFileSync(packagePath, JSON.stringify(pkg, null, 2) + "\n", "utf8");

console.log("TibiaGame patch runner installed.");
console.log("");
console.log("Future small patches:");
console.log("  1. Put apply-tibiagame-v*.mjs in the repository root.");
console.log("  2. Run: npm run patch");
console.log("");
console.log("Check only:");
console.log("  npm run patch:check");
console.log("");
console.log("Specific patch:");
console.log("  npm run patch -- .\\apply-tibiagame-v35.4-character-paperdoll.mjs");
